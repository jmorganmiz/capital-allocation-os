...
test('Document extraction produces cited proposals and pauses before calculation', () => {
  ...
  assert.match(extraction, /development or redevelopment site with no existing operating multifamily improvements as land/)
  assert.match(extraction, /existing legal multifamily unit count/)
...
test('Completed underwriting produces sensitivities and a sourced IC memo', () => {
  ...
  assert.match(memo, /Source appendix/i)
  assert.match(memo, /Approved screening assumptions/)
  assert.match(memo, /Levered IRR sensitivity/)
  assert.match(memo, /Model limitations/)
  assert.match(memo, /analyst approved/)
  assert.match(memo, /extraction confidence/)
  assert.match(memo, /grouped = new Map/)
  assert.match(memo, /formatDate/)
  assert.doesNotMatch(memo, /Package approved: \$\{String/)
