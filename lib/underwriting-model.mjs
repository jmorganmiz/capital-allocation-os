export const UNDERWRITING_MODEL_VERSION = 'va-multifamily-0.2.0'
...
function xnpv(rate, cashflows, dates) {
  ...
  return cashflows.reduce((total, cashflow, index) => {
    const elapsedYears = (dates[index].getTime() - start) / (365 * 24 * 60 * 60 * 1000)
    return total + cashflow / ((1 + rate) ** elapsedYears)
  }, 0)
}

export function calculateXirr(cashflows, dates, guess = 0.1) {
  if (!Array.isArray(cashflows) || !Array.isArray(dates) || cashflows.length !== dates.length) return null
  if (cashflows.length < 2 || !cashflows.some((value) => value < 0) || !cashflows.some((value) => value > 0)) return null

  let rate = guess
  const start = dates[0].getTime()
  for (let iteration = 0; iteration < 200; iteration += 1) {
    const value = xnpv(rate, cashflows, dates)
    const derivative = cashflows.reduce((total, cashflow, index) => {
      const elapsedYears = (dates[index].getTime() - start) / (365 * 24 * 60 * 60 * 1000)
      return total - (elapsedYears * cashflow) / ((1 + rate) ** (elapsedYears + 1))
    }, 0)
    if (!Number.isFinite(value) || !Number.isFinite(derivative) || derivative === 0) break
    const next = rate - value / derivative
    if (!Number.isFinite(next) || next <= -0.999999) break
  ...
  let low = -0.5
  let high = 2
  let lowValue = xnpv(low, cashflows, dates)
  let highValue = xnpv(high, cashflows, dates)
  for (const widerHigh of [5, 10]) {
    if (lowValue * highValue <= 0) break
    high = widerHigh
    highValue = xnpv(high, cashflows, dates)
  }
  if (lowValue * highValue > 0) return null

  for (let iteration = 0; iteration < 120; iteration += 1) {
    const midpoint = (low + high) / 2
    const midpointValue = xnpv(midpoint, cashflows, dates)
    if (Math.abs(midpointValue) < 1e-8 || high - low < 1e-10) return midpoint
    if (lowValue * midpointValue <= 0) {
      high = midpoint
      highValue = midpointValue
    } else {
...
export function runUnderwriting(input) {
  ...
  const renovationTargetUnits = normalized.unitMix.reduce(
    (total, row) => total + Math.min(row.units, row.unitsToRenovate ?? row.units),
    0,
  )
  const noiByYear = []
  ...
  for (let year = 1; year <= projectionYears; year += 1) {
    // Renovations complete at year-end, so units completed during a year begin
    // earning renovated rent in the following projection year. This matches the
    // source workbook's EOP renovation schedule.
    const unitsEarningRenovatedRent = Math.min(
      renovationTargetUnits,
      Math.max(0, normalized.unitsRenovatedPerYear * (year - 1)),
    )
    let grossRent = 0

    for (const row of normalized.unitMix) {
      const rowTarget = Math.min(row.units, row.unitsToRenovate ?? row.units)
      const renovatedUnits = renovationTargetUnits
        ? Math.min(row.units, unitsEarningRenovatedRent * (rowTarget / renovationTargetUnits))
        : 0
      const inPlaceUnits = row.units - renovatedUnits
      const inPlaceRent = row.currentRent * ((1 + normalized.rentGrowthInPlace) ** (year - 1))
      const renovatedRent = (row.marketRent + (row.renovationPremium ?? 0))
        * ((1 + normalized.rentGrowthRenovated) ** (year - 1))
      grossRent += (inPlaceUnits * inPlaceRent + renovatedUnits * renovatedRent) * 12
    }
  ...
  const capexByYear = Array.from({ length: hold }, (_, index) => {
    const year = index + 1
    const cumulativeBefore = Math.min(renovationTargetUnits, normalized.unitsRenovatedPerYear * index)
    const cumulativeAfter = Math.min(renovationTargetUnits, normalized.unitsRenovatedPerYear * year)
    const renovatedUnits = Math.max(0, cumulativeAfter - cumulativeBefore)
  ...
  const projectionStart = new Date(`${normalized.projectionStartDate}T00:00:00Z`)
  if (Number.isNaN(projectionStart.getTime())) throw new RangeError('projectionStartDate must be an ISO date')
  const cashflowDates = Array.from({ length: hold + 1 }, (_, index) => {
    const date = new Date(projectionStart)
    date.setUTCFullYear(projectionStart.getUTCFullYear() + index)
    return date
  })
  const positiveDistributions = leveredCashflows.slice(1).reduce(
    (sum, value) => sum + Math.max(0, value),
    0,
  )
  const additionalEquityContributions = leveredCashflows.slice(1).reduce(
  ...
  return {
    ...
    leveredIrr: calculateXirr(leveredCashflows, cashflowDates),
    unleveredIrr: calculateXirr(unleveredCashflows, cashflowDates),
    equityMultiple: initialEquity
      ? positiveDistributions / (initialEquity + additionalEquityContributions)
      : null,
    ...
    cashFlowBeforeTax,
    warnings: [
      'Renovations complete at year-end and begin earning renovated rent in the following projection year.',
