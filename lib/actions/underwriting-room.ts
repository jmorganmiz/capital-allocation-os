...
const ADVANCED_ASSUMPTION_DEFAULTS: Record<string, Json> = {
  renovationDowntimeMonths: 1,
  propertyTaxReassessmentMonth: 0,
  reassessedAnnualPropertyTaxes: 0,
  refinanceEnabled: 0,
  refinanceMonth: 36,
  refinanceLtv: 0.65,
...
const TRACKED_ASSUMPTIONS = [
  ...
  { key: 'renovationDowntimeMonths', label: 'Renovation downtime', category: 'renovation', unit: 'months', confidence: 0.7 },
  { key: 'propertyTaxReassessmentMonth', label: 'Property tax reassessment', category: 'tax', unit: 'month', confidence: 0.6 },
  { key: 'reassessedAnnualPropertyTaxes', label: 'Reassessed annual property taxes', category: 'tax', unit: '$/year', confidence: 0.6 },
  { key: 'refinanceEnabled', label: 'Refinance enabled', category: 'debt', unit: '0/1', confidence: 0.7 },
  { key: 'refinanceMonth', label: 'Refinance timing', category: 'debt', unit: 'month', confidence: 0.6 },
  { key: 'refinanceLtv', label: 'Refinance LTV', category: 'debt', unit: '%', confidence: 0.6 },
  { key: 'refinanceInterestRate', label: 'Refinance interest rate', category: 'debt', unit: '%', confidence: 0.6 },
  { key: 'constructionDrawAmount', label: 'Scheduled capital draw', category: 'construction', unit: '$', confidence: 0.6 },
  { key: 'constructionDrawMonth', label: 'Capital draw timing', category: 'construction', unit: 'month', confidence: 0.6 },
  { key: 'constructionLoanPct', label: 'Debt-funded draw share', category: 'construction', unit: '%', confidence: 0.6 },
  { key: 'incomeTaxRate', label: 'Income tax rate', category: 'tax', unit: '%', confidence: 0.5 },
  { key: 'capitalGainsTaxRate', label: 'Capital gains tax rate', category: 'tax', unit: '%', confidence: 0.5 },
  { key: 'depreciationRecaptureTaxRate', label: 'Depreciation recapture rate', category: 'tax', unit: '%', confidence: 0.5 },
  { key: 'waterfallEnabled', label: 'Waterfall enabled', category: 'waterfall', unit: '0/1', confidence: 0.7 },
  { key: 'lpEquityShare', label: 'LP equity share', category: 'waterfall', unit: '%', confidence: 0.7 },
  { key: 'preferredReturn', label: 'Preferred return', category: 'waterfall', unit: '%', confidence: 0.7 },
...
async function buildStepResult(
  ...
  if (step.step_key === 'assumption_review') {
    ...
    const assumptionRecords = TRACKED_ASSUMPTIONS.map((definition) => ({
      ...
      value: input[definition.key] ?? ADVANCED_ASSUMPTION_DEFAULTS[definition.key] ?? null,
